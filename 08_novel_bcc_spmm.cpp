...
void phase2_bcc_spmm(const Graph& g, const BCCDecomposition& bcc,
    ...
    for (int b = 0; b < nb; b++) {
        ...
        for (int li = 0; li < lb.K; li++) lb.g2l_vec[lb.l2g[li]] = li;

        // Build local adjacency
        lb.ladj.resize(lb.K);
        for (int li = 0; li < lb.K; li++) {
            int gu = lb.l2g[li];
            for (int gv : g.adj[gu]) {
                int lv = lb.g2l_vec[gv];
                if (lv >= 0) lb.ladj[li].push_back(lv);
            }
        }

        // Articulation points
        lb.art_global = bcc.bcc_arts[b];
        lb.num_arts = (int)lb.art_global.size();
        lb.art_local.resize(lb.num_arts);
        for (int a = 0; a < lb.num_arts; a++)
            lb.art_local[a] = lb.g2l_vec[lb.art_global[a]];
        lb.art_local_set = set<int>(lb.art_local.begin(), lb.art_local.end());

        // Home count = non-art nodes
        lb.home_count = lb.K - lb.num_arts;

        /* ── Bitwise multi-source BFS (OR-semiring SpMM) ── */
        lb.intra_sum.assign(lb.K, 0LL);

        for (int s_base = 0; s_base < lb.K; s_base += 64) {
            int batch_sz = min(64, lb.K - s_base);

            vector<uint64_t> frontier(lb.K, 0ULL);
            vector<uint64_t> visited(lb.K, 0ULL);
            vector<uint64_t> nextF(lb.K, 0ULL);

            for (int s = s_base; s < s_base + batch_sz; s++) {
                uint64_t bit = 1ULL << (s - s_base);
                frontier[s] = bit;
                visited[s]  = bit;
            }
            ...
            while (active) {
                ...
                // Update frontier and accumulate distances
                for (int u = 0; u < lb.K; u++) {
                    uint64_t nw = nextF[u] & ~visited[u];
                    if (nw != 0ULL) {
                        visited[u] |= nw;
                        frontier[u] = nw;
                        lb.intra_sum[u] += (long long)__builtin_popcountll(nw) * level;
                        active = true;
                    } else {
                        frontier[u] = 0ULL;
                    }
        ...
        /* ── Distances from every node to each articulation point ── */
        lb.dist_to_art.assign(lb.K, vector<int>(lb.num_arts, -1));
        for (int a = 0; a < lb.num_arts; a++) {
            vector<int> d = bfs_local(lb, lb.art_local[a]);
            for (int v = 0; v < lb.K; v++)
                lb.dist_to_art[v][a] = d[v];
        }

        /* ── Precompute art_home_sum[a] = Σ dist(art_a, v) for HOME v ── */
        lb.art_home_sum.assign(lb.num_arts, 0LL);
        for (int a = 0; a < lb.num_arts; a++) {
            for (int v = 0; v < lb.K; v++) {
                if (lb.art_local_set.count(v)) continue;  // skip art nodes
