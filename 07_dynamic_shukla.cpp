...
//phase 2: BCC-confined BFS (Shukla-style, no hardware vectorization)
void phase2_bcc_bfs(const Graph& g, const BCCDecomposition& bcc,
    ...
    for (int b = 0; b < nb; b++) {
        ...
        // Compute intra-BCC distance sums using standard BFS per source
        lb.intra_sum.assign(lb.K, 0);
        for (int s = 0; s < lb.K; s++) {
            vector<int> dist = bfs_local(lb, s);
            for (int t = 0; t < lb.K; t++) {
                if (dist[t] > 0)
                    lb.intra_sum[s] += dist[t];
            }
        }

        // Compute distances from each node to each articulation point
        lb.dist_to_art.assign(lb.K, vector<int>(lb.num_arts, -1));

        for (int a = 0; a < lb.num_arts; a++) {
            vector<int> dist = bfs_local(lb, lb.art_local[a]);
            for (int i = 0; i < lb.K; i++)
...
int main(int argc, char* argv[]) {
    ...
    //phase 2: BCC-confined BFS
    vector<LocalBCC> lccs;
    phase2_bcc_bfs(g, bcc, lccs);
    auto t3 = chrono::high_resolution_clock::now();
    printf("[Phase 2]  BCC-BFS (parallel): %.2f ms\n",
